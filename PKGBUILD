...
source=("https://downloads.ferronweb.org/${pkgver}/ferron-${pkgver}-${_arch}-unknown-linux-gnu.zip")
sha256sums=()
provides=('ferron')

prepare() {
    cd "$srcdir"
    unzip -q "ferron-$pkgver-${_arch}-unknown-linux-gnu.zip"
}
...
package() {
    ...
    install -Dm755 ferron "$pkgdir/usr/sbin/ferron"
